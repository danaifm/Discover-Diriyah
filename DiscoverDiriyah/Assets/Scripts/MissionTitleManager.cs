        ...
        collectionRef.GetSnapshotAsync().ContinueWithOnMainThread(task => {
            ...
            foreach (DocumentSnapshot document in snapshot.Documents) {
                ...
                if (document.Exists)
                {
                    ...
                    string question = document.GetValue<string>("Question");
                    string correctAnswer = document.GetValue<string>("CorrectAnswer");
                    string wrongAnswer1 = document.GetValue<string>("WrongAnswer1");
                    string wrongAnswer2 = document.GetValue<string>("WrongAnswer2");
                    dateTimeStrs.Add(document.GetValue<string>("Time"));
                    mission.GetComponentInChildren<ViewMissions>().SetTitle(title);
                    mission.GetComponentInChildren<ViewMissions>().SaveTexts(title, question, correctAnswer, wrongAnswer1, wrongAnswer2);
                    Debug.Log($"initialized {i+1}");
                    // DateTime.ParseExact(document.GetValue<string>("Time"), "dd-MM-yyyy, HH:mm:s", CultureInfo.InvariantCulture)
                }
                else
                {
